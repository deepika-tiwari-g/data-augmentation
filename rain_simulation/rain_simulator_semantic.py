...
class RainSimulator:
    ...
    def generate_rain_streaks(self, image: np.ndarray) -> np.ndarray:
        ...
        # ENHANCED: More rain streaks for visibility (matching reference)
        num_streaks = int(self.rain_intensity * 5000 * (h * w) / (1920 * 1080))
        ...
        for _ in range(num_streaks):
            ...
            # ENHANCED: Higher opacity for more visible rain (matching reference)
            opacity = random.uniform(0.15, 0.30)
    ...
    def add_atmospheric_effects(self, image: np.ndarray) -> np.ndarray:
        ...
        # FIXED: Reduced atmospheric haze to preserve colors
        # More subtle gradient
        depth_gradient = np.linspace(self.atmospheric_depth * 0.6, self.atmospheric_depth * 0.2, h)
        ...
        # FIXED: Reduced volumetric mist to preserve colors
        mist = self._generate_mist(h, w)
        mist_strength = self.atmospheric_depth * 0.25  # Reduced from 0.35
    ...
    def apply_color_grading(self, image: np.ndarray) -> np.ndarray:
        ...
        # FIXED: Reduced desaturation to preserve colors (was 35%, now 15%)
        hsv[:, :, 1] *= (1.0 - self.color_temperature * 0.15)
        
        # Convert back to BGR
        img_float = cv2.cvtColor(hsv, cv2.COLOR_HSV2BGR)
        img_float = np.clip(img_float, 0.0, 1.0)
        
        # FIXED: Reduced cool color shift to preserve original colors
        color_shift = self.color_temperature * 0.06  # Reduced from 0.12
        img_float[:, :, 0] *= (1.0 + color_shift * 0.8)  # Blue channel (reduced boost)
        img_float[:, :, 1] *= (1.0 + color_shift * 0.2)  # Green channel (minimal)
        img_float[:, :, 2] *= (1.0 - color_shift * 0.4)  # Red channel (reduced reduction)
        
        # FIXED: Less darkening to preserve brightness
        img_float *= 0.92  # Reduced from 0.88
    ...
    def apply_vehicle_effects(self, image: np.ndarray, vehicle_mask: np.ndarray) -> np.ndarray:
        """Apply vehicle-specific rain effects"""
        if vehicle_mask.max() < 0.01:
            return image
    ...
    def _add_vehicle_droplets(self, image: np.ndarray, vehicle_mask: np.ndarray) -> np.ndarray:
        """Add water droplets on vehicles"""
        h, w = image.shape[:2]
        droplet_layer = image.copy()
        num_droplets = int(min(vehicle_mask.sum() / 100, 200))
    ...
    def apply_vegetation_effects(self, image: np.ndarray, vegetation_mask: np.ndarray) -> np.ndarray:
        """Apply vegetation-specific effects"""
        if vegetation_mask.max() < 0.01:
            return image
    ...
    def _create_wet_surface_semantic(self, image: np.ndarray, regions: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        # FIXED: Further reduced darkening to preserve original colors
        total_darkening = regions['road'] * 0.18 + regions['vehicles'] * 0.10 + regions['vegetation'] * 0.15
        total_darkening_3ch = np.stack([total_darkening] * 3, axis=-1)
        img_float = img_float * (1.0 - total_darkening_3ch)
        
        # Enhanced contrast for road areas (subtle)
        road_mask_3ch = np.stack([regions['road']] * 3, axis=-1)
        road_contrast = (img_float - 0.5) * 1.2 + 0.5
        ...
        img_float[blend_start:, :] = np.clip(
            img_float[blend_start:, :] * (1 - fade * road_lower_3ch) + reflection * fade * road_lower_3ch, 0.0, 1.0)
    ...
    def _generate_puddles_semantic(self, image: np.ndarray, road_mask: np.ndarray) -> np.ndarray:
        ...
        result_float = result_float * (1 - puddle_mask_3ch * 0.5) + ripple_float * puddle_mask_3ch * 0.5
    ...
    def _add_atmospheric_effects_semantic(self, image: np.ndarray, regions: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        img_float = img_float * (1 - mist * mist_strength) + fog_color * mist * mist_strength
    ...
    def _apply_rain_streaks_semantic(self, image: np.ndarray, regions: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        result_float = image.astype(np.float32) / 255.0
        # ENHANCED: Brighter rain color for better visibility (matching reference)
        rain_color = np.array([0.92, 0.92, 0.92], dtype=np.float32)
        rain_mask_3ch = np.stack([rain_mask] * 3, axis=-1)
        result_float = result_float * (1.0 - rain_mask_3ch) + rain_color * rain_mask_3ch
        
        return (np.clip(result_float, 0.0, 1.0) * 255).astype(np.uint8)
    
    def simulate_rain(self, image: np.ndarray) -> np.ndarray:
        """Apply complete rain simulation (semantic-aware or uniform)"""
    ...
    def _simulate_rain_uniform(self, image: np.ndarray) -> np.ndarray:
        """Original uniform rain simulation"""
        result = self.apply_color_grading(image)
        ...
        ripple_float = ripple_layer.astype(np.float32) / 255.0
        result_float = result_float * (1 - puddle_mask_3ch * 0.4) + ripple_float * puddle_mask_3ch * 0.4
        result = (np.clip(result_float, 0.0, 1.0) * 255).astype(np.uint8)
