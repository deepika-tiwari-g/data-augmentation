#!/usr/bin/env python3
"""
Script to add semantic-aware methods to rain_simulator_semantic.py
"""

# Read the original file
with open('rain_simulator_semantic.py', 'r') as f:
    lines = f.readlines()

# Find the line number where simulate_rain starts (around line 647)
insert_line = None
for i, line in enumerate(lines):
    if 'def simulate_rain(self, image: np.ndarray)' in line:
        insert_line = i
        break

if insert_line is None:
    print("Error: Could not find simulate_rain method")
    exit(1)

print(f"Found simulate_rain at line {insert_line + 1}")

# New semantic-aware methods to insert
new_methods = '''    
    ...
    def _add_vehicle_droplets(self, image: np.ndarray, vehicle_mask: np.ndarray) -> np.ndarray:
        ...
        for _ in range(min(num_droplets, 200)):  # Cap at 200 droplets
    ...
    def _create_wet_surface_semantic(self, image: np.ndarray, regions: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        Returns:
            Image with region-specific wet effects (uint8)
        """
        img_float = image.astype(np.float32) / 255.0
    ...
    def _generate_puddles_semantic(self, image: np.ndarray, road_mask: np.ndarray) -> np.ndarray:
        ...
        Args:
            image: Input image (uint8)
            road_mask: Road region mask [0.0-1.0]
            
        Returns:
            Image with puddles (uint8)
        """
...
# Insert the new methods before simulate_rain
lines.insert(insert_line, new_methods)

# Now replace the simulate_rain method with the semantic-aware version
# Find where simulate_rain ends
simulate_rain_end = None
for i in range(insert_line + len(new_methods.split('\n')), len(lines)):
    # Look for the next method definition or end of class
    if lines[i].strip().startswith('def ') and 'simulate_rain' not in lines[i]:
        simulate_rain_end = i
        break

if simulate_rain_end is None:
    print("Error: Could not find end of simulate_rain method")
    exit(1)

print(f"simulate_rain ends at line {simulate_rain_end + 1}")

# Replace simulate_rain with semantic-aware version
new_simulate_rain = '''    def simulate_rain(self, image: np.ndarray) -> np.ndarray:
        """
...
# Replace the old simulate_rain with the new one
del lines[insert_line + len(new_methods.split('\n')):simulate_rain_end]
lines.insert(insert_line + len(new_methods.split('\n')), new_simulate_rain)

# Write the modified file
with open('rain_simulator_semantic.py', 'w') as f:
    f.writelines(lines)

print("Successfully added semantic-aware methods to rain_simulator_semantic.py")
print(f"Added {len(new_methods.split(chr(10)))} lines of new methods")
